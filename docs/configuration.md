...
| `ORCHESTRATOR_MODEL` | `google/gemini-3.8-flash` | Model for all agent nodes **when `LLM_BACKEND=openrouter`**; not applied in the default local mode, where `OLLAMA_LLM_MODEL` decides. `anthropic/claude-sonnet-4.6` until 2026-09-18; the documented backup is `anthropic/claude-sonnet-5` with `PRICE_IN_PER_MTOK=2.0` / `PRICE_OUT_PER_MTOK=10.0` ([the comparison](eval-results/2026-09-18-hosted-models.md)) |
