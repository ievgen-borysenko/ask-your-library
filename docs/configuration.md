...
| `CHAPTER_SCAN_CHARS` | `120000` | How much of a chapter that window may be chosen from. A scan budget, not an observation budget: the model still sees `CHAPTER_HIT_CHARS`. Must be at least `CHAPTER_HIT_CHARS`, or startup refuses. 120,000 covers 1,242 of the demo corpus's 1,245 sections whole |
...
| `ASK_SCRATCH_DIR` | `$AYL_HOME/scratch` | Where raw-hit scratchpads are written (a human-readable log; `validate` does not read it), by `ayl ask` and by the server `ayl ui` starts alike. `~` is expanded and the path made absolute, so neither writes the passages beside the caller's working directory — which the CLI's old default, a relative `.scratch`, did |
...
| `AYL_CHAINLIT_DIR` | `$AYL_HOME/ui/.chainlit/` | Where the UI writes its chat db and auth secret. Unset, a checkout whose `.chainlit/` already holds a `chat.db` from before that default moved keeps it there, with one line when the web chat starts, until 0.5.0 ([upgrading](upgrading.md#the-index-moved-to-ayl_homeindex)). `~` is expanded and the path resolved, once, in one place (`ui/launcher.py`), so the web chat and `ayl backup` / `ayl restore` cannot end up at two different directories — never the working directory, which anyone can prepare a `.chainlit/auth-secret` in; tests and the canary point it at a temp dir. It also decides the **Chainlit app root** `ayl ui` prepares — the directory above it — into which the launcher writes `chainlit_config.toml` and the translation on every start; unset, that root is `$AYL_HOME/ui`. `ayl backup` / `ayl restore` read the same variable to find `chat.db` (and `--chat-db <path>` overrides it) |
