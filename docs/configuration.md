...
| `MAX_DROPPED_STREAK` | `2` | How many steps in a row may lose every quote to the provenance gate before such a step counts as dry. A step whose passages were retrieved but never quoted verbatim is not silence from the library, so the first ones do not advance the CRAG gate above; this bounds what a model that keeps retrieving and never copying may spend |
